...
export default function BlogPost() {
        ...
        <main className="container mx-auto px-4 md:px-6 pt-24 md:pt-28 pb-8 md:pb-12">
          <ScrollFadeIn>
            <div className="text-center py-12">
              <h1 className="text-2xl md:text-3xl font-semibold mb-4">Post Not Found</h1>
              <p className="text-gray-400 text-sm md:text-base mb-8">The blog post you're looking for doesn't exist.</p>
              <Link
                to="/blog"
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      {/* Main Content */}
      <main className="container mx-auto px-4 md:px-6 pt-24 md:pt-28 pb-8 md:pb-12 max-w-4xl">
        {/* Back Button */}
        <ScrollFadeIn>
          <Link
            to="/blog"
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-4 text-gray-400 flex-wrap pt-2">
              <div className="flex items-center gap-2">
                ...
                <time dateTime={post.publishedDate} className="text-xs">
              ...
              <span className="text-gray-600">•</span>
              <span className="text-xs">By Ethan Clinick</span>
            </div>
          ...
          <article className="prose prose-invert max-w-none">
            <div className="markdown-content">
              <ReactMarkdown
                components={{
                  h1: ({ node, ...props }) => (
                    <h1 className="text-2xl font-semibold mt-8 mb-4 text-white" {...props} />
                  ),
                  h2: ({ node, ...props }) => (
                  ...
                  p: ({ node, ...props }) => (
                    <p className="text-gray-300 text-sm md:text-base leading-relaxed mb-4" {...props} />
                  ),
                  code: ({ node, ...props }: ComponentProps<'code'> & ExtraProps) => {
                    ...
                    return (
                      <code className="block bg-gray-900 p-4 rounded-lg overflow-x-auto text-sm mb-4" {...props} />
                    );
                  },
                  pre: ({ node, ...props }) => (
                    <pre className="bg-gray-900 p-4 rounded-lg overflow-x-auto text-sm mb-4" {...props} />
                  ),
                  a: ({ node, ...props }) => (
                    <a className="text-orange-500 hover:text-orange-400 underline" {...props} />
                  ),
                  ul: ({ node, ...props }) => (
                    <ul className="list-disc list-inside mb-4 text-gray-300 text-sm md:text-base space-y-2" {...props} />
                  ),
                  ol: ({ node, ...props }) => (
                    <ol className="list-decimal list-inside mb-4 text-gray-300 text-sm md:text-base space-y-2" {...props} />
                  ),
                  li: ({ node, ...props }) => (
                    <li className="text-gray-300" {...props} />
                  ),
                  blockquote: ({ node, ...props }) => (
                    <blockquote className="border-l-4 border-orange-500 pl-4 italic text-gray-400 text-sm md:text-base my-4" {...props} />
                  ),
                  strong: ({ node, ...props }) => (
